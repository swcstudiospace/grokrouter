...
- **Grok Bot 0.44.0 support.** The patcher now selects a per-version manifest from `patch/manifests/` by exact stock hash or anchor match, so one payload serves 0.30.0 and 0.44.0 without loosening either version's gates. The 0.44.0 host reads the mock response from executor options instead of the environment; the session hook accepts both spellings and every other anchor is unchanged. Live 0.44.0 acceptance is pending.
- **Router errors now say why.** A failed turn is classified into a short code with a one-line fix, shown in the Bot as `Model Router error [rate-limit]` rather than a bare sentence. `/router doctor` and `grokbot-router doctor` list the three most recent failures, and the new `grokbot-router errors` prints the history. The reason was already in the audit log; nothing surfaced it before.
- **Fixed: xAI 400 on every model.** xAI rejects `reasoning_effort` on models that do not support it, and reports the field in camelCase (`does not support parameter reasoningEffort`). The retry guard compared snake_case names and the phrase "not supported", so it matched neither and never stripped the field. Field names are now matched across both spellings, a rejected field is remembered per model, and a `bad-request` message now repeats what the provider actually said instead of generic advice.
- **New: `grokbot-router probe xai`** reports which request shapes the account accepts across both xAI hosts, printing no credential.
- **Fewer failures to diagnose.** A 429 or 5xx is retried once honouring `Retry-After`, a 400 that names an optional request field is retried once without those fields, and a model with no native tool support is no longer sent tool schemas.
- **Fixed: Anthropic sign-in ran the wrong native binary.** The helper globbed for any `claude-agent-sdk-*` build and could pick the musl one on a glibc Bot computer, failing with `cannot execute: required file not found`. The installer now verifies the binary for this platform and C library, records its path, and the helper proves it runs before offering sign-in.
- **Fixed: host-side redaction missed OAuth bearers.** The patch's audit redaction covered API keys but not xAI tokens or JWTs.
- **Live model lists for every provider.** `/models` now shows the provider's real catalog rather than a packaged shortlist: OpenRouter's public list, xAI's authenticated list across both the metered API and the subscription proxy, and Anthropic's own list from the Claude Agent SDK. `/models refresh` re-reads it on demand, and each list is cached for an hour with a packaged fallback. The shipped defaults were refreshed to Claude Sonnet 5, Opus 5, Haiku 4.5, GPT-6 Astra, and the current Grok models.
