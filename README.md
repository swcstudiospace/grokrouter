...
| xAI keeps failing with `bad-request` | The message now repeats xAI's own words. If it names a parameter, the router drops that field and remembers it. For the full picture run `grokbot-router probe xai` in the Bot computer, which reports exactly which request shapes your account accepts. |
